...
(function () {
  ...
  /* Fetch featured from static data */
  var allData = typeof STATIC_PRODUCTS !== 'undefined' ? STATIC_PRODUCTS : [];
  var featured = [];
  for (var fi = 0; fi < allData.length; fi++) {
    if (allData[fi].featured) featured.push(allData[fi]);
  }
  if (featured.length > 0) {
    for (var fi2 = 0; fi2 < featured.length; fi2++) buildSlide(featured[fi2], fi2);
  }
  ...
  (function() {
    ...
    quizProducts = typeof STATIC_PRODUCTS !== 'undefined' ? STATIC_PRODUCTS : [];
    renderQuiz();
